  ...
  <div class="informs">
    <el-card class="informs-card">
      ...
      <span style="float: left; margin-bottom: 1rem; width: 66%;" class="btn-group">
        <el-button size="small" @click="showcustInfoDialog">新增</el-button>
      ...
      <el-table :data="informs" style="width: 100%;" v-loading="loading" element-loading-text="给我一点时间" border @selection-change="handleSelectionChange" :header-cell-style="headerStyle">
        ...
        <el-table-column prop="singleNum" label="客户编号" align="center"></el-table-column>
        <el-table-column prop="material" label="名称" align="center"></el-table-column>
        ...
        <el-table-column prop="prod" label="业务员" align="center"></el-table-column>
    ...
    <update-table-for-cust-info :visible="custInfoVisible" @cancel="custInfoDialog" @confirm="custInfoUpdate" />
  </div>
</template>
<script>
import { fetchInformByPage } from '@/api/inform/admin'
import SearchButton from '@/components/Buttons/SearchButton'
import UpdateTableForCustInfo from '@/components/Dialog/UpdateTableForCustInfo'
...
export default {
  ...
  methods: {
    ...
    // 子组件传数据
    showcustInfoDialog() {
      this.custInfoTemp = this.informs
      this.custInfoDialog()
    },
    // 子组件显示
    custInfoDialog() {
      console.log(this.custInfoVisible)
      this.custInfoVisible = !this.custInfoVisible
    },
    // 子组件更新数据按钮
    custInfoUpdate() {
      this.custInfoDialog()
