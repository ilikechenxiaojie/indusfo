<template>
  <div class="informs">
    <el-card class="informs-card">
      <div slot="header" class="clearfix" style="height: 2.4rem;">
        <span style="float: left; text-align: left; width:30%; color:#bbb; font-size: 0.8rem;">
          <i class="el-icon-info"></i>&nbsp;&nbsp;原材料资料
          <br>
          用于维护原材料的基础资料(注:要保证供应商编号的唯一性,不可重复)
          <hr style=" margin: 0.6rem 0;">
        </span>
      </div>
      <span style="float: left; margin-bottom: 1rem">
        <el-button size="small" @click="showMaterialDialog">新增</el-button>
      ...
      <el-table :data="informs" style="width: 100%;" v-loading="loading" element-loading-text="给我一点时间" border @selection-change="handleSelectionChange" :header-cell-style="headerStyle">
        ...
        <el-table-column prop="singleNum" label="原材料编号" align="center"></el-table-column>
        <el-table-column prop="material" label="品名材质" align="center"></el-table-column>
        <el-table-column prop="type" label="类型" align="center"></el-table-column>
        <el-table-column prop="prod" label="代表验收产品" align="center"></el-table-column>
        <el-table-column prop="ways" label="检测方法" align="center">
    ...
    <update-table-for-recheck :data="temp" :visible="visible" @cancel="toggleDialog" @confirm="handleUpdate" />
    <update-table-for-material :data="materialTemp  " :visible="materialVisible" @cancel="materialDialog" @confirm="materialUpdate" />
  </div>
</template>
<script>
import { fetchInformByPage } from '@/api/inform/admin'
import SearchButton from '@/components/Buttons/SearchButton'
import UpdateTableForRecheck from '@/components/Dialog/UpdateTableForRecheck'
import UpdateTableForMaterial from '@/components/Dialog/UpdateTableForMaterial'
...
export default {
  ...
  methods: {
    ...
    showMaterialDialog() {
      this.materialTemp = this.informs
      this.materialDialog()
    },
    // 子组件显示
    materialDialog() {
