<template>
  <div class="informs">
    <el-card class="informs-card">
      <div slot="header" class="clearfix" style="height: 2.4rem;">
        <span style="float: left; text-align: left; width:30%; color:#bbb; font-size: 0.8rem;">
          <i class="el-icon-info"></i>&nbsp;&nbsp;设备资料
          <br>
          用于维护客户的基础资料(注:要保证客户编号的唯一性, 不可重复)
          <hr style=" margin: 0.6rem 0;">
        </span>
      </div>
      <span style="float: left; margin-bottom: 1rem; width: 66%;" class="btn-group">
        <el-button size="small" @click="showequipmentDialog">新增</el-button>
    ...
    <update-table-for-equipment :visible="equipmentVisible" @cancel="equipmentDialog" @confirm="equipmentUpdate" />
  </div>
</template>
<script>
import { fetchInformByPage } from '@/api/inform/admin'
import SearchButton from '@/components/Buttons/SearchButton'
import UpdateTableForEquipment from '@/components/Dialog/UpdateTableForEquipment'
...
export default {
  ...
  methods: {
    ...
    // 子组件传数据
    showequipmentDialog() {
      this.equipmentTemp = this.informs
      this.equipmentDialog()
    },
    // 子组件显示
    equipmentDialog() {
      console.log(this.equipmentVisible)
